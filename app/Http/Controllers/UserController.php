...
class UserController extends Controller {
	...
	public function getIndex()
	{
		...
		{
			$user=User::where('company_id','=',Auth::user()->company->id)->orderBy('updated_at', 'desc')->paginate(10);
		}
		else
		{
	...
	public function getCreateuser()
	{
		...
		$data=array(
			...
			'admin_company'=>array(Auth::user()->company->id=>Auth::user()->company->title),
			'role_list'=>$role_list,
	...
	{	
		...
		if($validator->fails()){
			...
			if($user->save())
			{
				...
				$employee->save();
				$message=$globalFunctions->globalMessage(' User has been created Successfully!.','success');
				
			}
			else{
				$message=$globalFunctions->globalMessage('Company has not created successfully!.','danger');
	...
	public function getUserprofile(){ 
		...
		if($employee->dob=='0000-00-00 00:00:00' || $employee->dob==null)
		{
			$employee->dob="";
		...
		{		
			...
			$filename="";
				...
				if($employee->save())
				{
					...
					$message=$globalFunctions->globalMessage('Userprofile has been updated Successfully!.','success');
	...
	public function postUpdateuser($id)
	{
		...
		if($user->save())
		{
			...
			$employee->designation_id=Input::get('designation_id');
			$employee->save();
			$message=$globalFunctions->globalMessage('User has been updated Successfully!.','success');
			
		}
		else{
			$message=$globalFunctions->globalMessage('User has not been updated Successfully!.','success');
